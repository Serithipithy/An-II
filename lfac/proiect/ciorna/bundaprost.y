...
void declarare_array_init(char* tip,char* nume,int dimensiune1,int dimensiune2,char* elemente){
   ...
   while(strlen(elemente)>0){
     char nr[10]; int k=0;
     while(elemente[0]!=','&&strlen(elemente)>0){
         nr[k]=elemente[0];k++;
         printf("elemente ianinte de strcpy while: %s,%d\n",elemente,strlen(elemente));
         strcpy(elemente,elemente+1);
         elemente[strlen(elemente)-1]='\0';
         printf("elemente dupa strcpy while: %s\n",elemente);
     }
     printf("elemente dupa while1: %s\n",elemente);
     strcpy(elemente,elemente+1);
     printf("elemente while2: %s\n",elemente);
     int x;
     strcpy(nr, nr+1);
     nr[strlen(nr)-1]='\0';
     x=atoi(nr);
