...
int functii_declarate(char* nume)
{
    for(int i=0;i<f;i++)
    {
       if(strcmp(functii[i].nume,nume)==0) return i;
    }
    return -1;
}
void declarare_functie(char* tip, char* nume,char* args )
{
    if(functii_declarate(nume)!=-1)
    {
       char buffer[200];
...
int aparitii(char* cauta, char ch){
  int i;
  int k=0;

  for( i=0; i< strlen(cauta); i++ )
  if(cauta[i] == ch) k++;
  return k;
}
void call_function (char* nume, char* variabile){
  int poz = functii_declarate(nume);
  if(poz ==-1)
    {
       char buffer[200];
       sprintf(buffer,"Nu exista functia cu numele %s",nume);
       yyerror(buffer);
       exit(0);
    }
    char tipuri[300];
    strcpy(tipuri,functii[poz].argumente);

    if( aparitii(tipuri,',') != aparitii(variabile,','))
    {
      char buffer[200];
       sprintf(buffer,"Prea multe sau prea putine argumente date functiei cu numele %s",nume);
       yyerror(buffer);
       exit(0);
    }
}
...
%type <str> TIP ID CHAR STRING BOOL NR signatura parametrii elemente operanzi
...
declaratie: TIP ID SEMICOLON /*int a; sau int a,b,c;//functie de declarare fara valoare*/ {declarare_fara_initializare($1,$2,0);}
          ...
          | ID LPARAN operanzi RPARAN SEMICOLON {call_function($1,$3);}
          | TIP ID signatura /* pt functii si clase*/{declarare_functie($1,$2,$3);}
          | array SEMICOLON
                ;               
operanzi: ID {$$ = $1;}
        | operanzi COMMA ID {$$ = $1; strcat($$,",");strcat($$,$3);}
        ;                       
